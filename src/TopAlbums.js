...
const TopAlbumsComponent = () => {
    ...
    const displayedSortedAlbums = showMore ? sortedAlbums : sortedAlbums.slice(0, 5)


    return (
        <div className="p-6 max-w-4xl mx-auto bg-[#3a0e73] rounded-lg shadow-lg  ">
            <h1 className="text-2xl font-bold text-center mb-6 text-white">Top Albums</h1>
            <button 
                    ...
                    {displayedSortedAlbums.map((entry, index) => {
                        ...
                        return (
                            <tr key={index} className={`hover:bg-gray-700 transition duration-300 ${medalColor}`}>
                                ...
                                <td className="py-4 px-4">{entry.album}</td>
